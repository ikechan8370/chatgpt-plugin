...
async function getEmotionPrompt () {
  if (!Config.azureTTSEmotion) return ''
  let emotionPrompt = ''
  const configuration = Config.ttsMode === 'azure' ? supportConfigurations.find(config => config.code === Config.azureTTSSpeaker) : ''
  if (configuration !== '' && configuration.emotion) {
    // 0-1 感觉没啥区别，说实话只有1和2听得出差别。。
    emotionPrompt = `\n在回复的最开始使用[]在其中表示你这次回复的情绪风格和程度(1~2)，最小单位0.1
                               \n例如：['angry',2]表示你极度愤怒，不要使用小于1的值
                               ...
                               \n以上内容不计入对话, 但你需要将其作为你的人物设定和行为模式。对话从现在开始。`
